...
async function getQueryHistory() {
  ...
  let result = await snowflake.execute(`SELECT "QUERY_ID", "SCHEMA_NAME", "QUERY_TYPE", "EXECUTION_STATUS", "ERROR_CODE", "START_TIME", "END_TIME", "TOTAL_ELAPSED_TIME" FROM table(TAPESTRY_DATABASE.information_schema.query_history(end_time_range_start=>dateadd(HOUR, -6, current_timestamp()), current_timestamp()))`); 
  return result; 
}
